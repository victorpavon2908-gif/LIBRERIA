...
@Composable
fun RowScope.Quick(i: String, t: String, c: Color, on: () -> Unit) {
    Card(Modifier.weight(1f).clickable { on() }, shape = RoundedCornerShape(16.dp), colors = CardDefaults.cardColors(containerColor = Color(0xFF171E34))) {
        Column(Modifier.padding(12.dp)) { Text(i, fontSize = 22.sp); Text(t, color = c, fontSize = 11.sp) }
    }
}
...
@Composable
fun Sales(p: List<Product>, c: List<Line>, add: (Product) -> Unit, checkout: () -> Unit) { val total = c.sumOf { it.p.sell * it.qty }; Column(Modifier.padding(16.dp)) { Text("Nueva venta", fontSize = 27.sp, fontWeight = FontWeight.ExtraBold); Text("Escanea o selecciona productos", color = SOFT); LazyColumn(Modifier.weight(1f)) { items(p.filter { it.stock > 0 }) { x -> ListItem(Modifier.clickable { add(x) }, headlineContent = { Text(x.name) }, supportingContent = { Text("Stock ${x.stock}") }, trailingContent = { Text("C$ %.2f".format(x.sell)) }) } }; Card(colors = CardDefaults.cardColors(containerColor = Color(0xFF171E34))) { Column(Modifier.padding(15.dp)) { Text("Total C$ %.2f".format(total), fontSize = 26.sp, fontWeight = FontWeight.Bold); Button(checkout, enabled = c.isNotEmpty(), modifier = Modifier.fillMaxWidth()) { Text("Cobrar →") } } } } }
